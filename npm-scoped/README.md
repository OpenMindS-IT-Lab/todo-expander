...
Transform simple TODO comments into structured, Codex-ready task briefs using
AI.
...
> - Legacy packages (@saladin/todo-expander on JSR and unscoped todo-expander on
>   NPM) have been deprecated/yanked.
> - If you previously installed the legacy NPM package globally, uninstall it
>   before installing the scoped one:
...
TODO Expander is a Deno-based CLI tool that automatically converts plain TODO
comments into detailed, structured specifications that are ready for AI coding
assistants. Instead of vague `// TODO: fix this later`, get comprehensive briefs
with Context, Goal, Steps, Constraints, and Acceptance criteria.
...
Todo-expander supports layered configuration with the following precedence
(highest to lowest):
...
Create a `.todoexpandrc.json` file in your project root with IntelliSense
support:
...
Use a repo-tracked hook to auto-format staged files with `deno fmt` before
committing:
...
The hook formats staged TS/JS/JSON/MD files and re-stages them so the commit
includes the changes. Create a `.no-format-hook` file at the repo root to
temporarily opt out.
