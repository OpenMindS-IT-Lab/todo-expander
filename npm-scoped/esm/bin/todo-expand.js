...
async function main() {
    ...
    if (dntShim.Deno.args[0] === 'init') {
        ...
        if (initFlags.help) {
            console.log(`todo-expand init - Initialize a project with todo-expander configuration
...
`);
        ...
        if (template && !['base', 'monorepo', 'non-git'].includes(template)) {
            console.error(yellow(`Invalid template: ${template}. Must be one of: base, monorepo, non-git`));
            dntShim.Deno.exit(1);
        }
        await initProject({
    ...
    const dryRun = flags['dry-run'] || dntShim.Deno.env.get('TODO_EXPAND_DRY') === '1';
    ...
    if (!apiKey) {
        console.error(yellow('OPENAI_API_KEY is not set. Export it before running (see Notebook Preflight).'));
        dntShim.Deno.exit(2);
    }
    // Determine targets
    ...
    for (const abs of targets) {
        ...
        try {
            ...
            processed++;
            if (cfg.verboseLogs) {
                console.log(gray(`[done]  ${rel}  (todos: ${ok.todosFound}, changed: ${ok.changed})`));
            }
            else {
