    ...
    try {
        ...
        for await (const entry of dntShim.Deno.readDir(cwd)) {
            if (entry.isDirectory &&
                ...
                const hasPackageJson = await exists(join(cwd, entry.name, 'package.json'));
                if (hasPackageJson) {
 ...
 */
function getTodoExpanderRoot() {
    // This will be the directory containing this script
    const scriptPath = new URL(globalThis[Symbol.for("import-meta-ponyfill-esmodule")](import.meta).url).pathname;
    return dirname(dirname(scriptPath)); // Go up from src/ to root
}
/**
 * Copy a template file to the target location
 */
async function copyTemplate(templatePath, targetPath, force) {
    ...
    if (!force && (await exists(targetPath))) {
        console.log(yellow(`Skipping ${targetPath} (already exists, use --force to overwrite)`));
        return false;
    }
    // Copy the template
 ...
 */
async function addNpmScript(cwd, force) {
    ...
    try {
        ...
        if (packageJson.scripts['todos:expand'] && !force) {
            console.log(yellow(`Skipping package.json script (already exists, use --force to overwrite)`));
            return false;
        }
        // Add the script
        packageJson.scripts['todos:expand'] = 'todo-expand --staged';
        // Write back with proper formatting
        await dntShim.Deno.writeTextFile(packageJsonPath, JSON.stringify(packageJson, null, 2) + '\n');
        return true;
    }
    catch (error) {
        console.warn(yellow(`Warning: Could not update package.json: ${error.message}`));
        return false;
    }
}
/**
 ...
 */
function printGuidance(template, createdFiles) {
