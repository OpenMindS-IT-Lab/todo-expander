...
async function loadTemplate() {
    ...
    const candidates = [
        join(dntShim.Deno.cwd(), 'prompts/todo_expander.compact.prompt.md'),
    ];
    for (const p of candidates) {
        try {
            const text = await dntShim.Deno.readTextFile(p);
 ...
 */
export async function renderPrompt({ filePath, language, todoComment, codeContext, style, sections, }) {
    ...
    if (tpl)
        ...
        parts.push('Task: Rewrite the TODO into a structured brief as a comment. Return only the rewritten comment.');
    }
...
export async function renderPromptBatch({ filePath, language, todos, style, sections, }) {
    const tpl = await loadTemplate();
    const defaultSections = [
        'Context',
        'Goal',
    ...
    todos.forEach((t, idx) => {
        parts.push(`TODO ${idx + 1}:`, t.todoComment, '', 'Nearby code (context only):', t.codeContext);
...
export async function runLLM({ prompt, apiKey, cfg }) {
    ...
    const attemptOnce = async () => {
        ...
        try {
            const res = await dntShim.fetch(cfg.endpoint, {
                method: 'POST',
                headers: {
    ...
    for (let attempt = 1; attempt <= totalAttempts; attempt++) {
        ...
        if (attempt < totalAttempts && retriable) {
            const delay = Math.min(5000, (cfg.retryBackoffMs ?? 500) * Math.pow(2, attempt - 1));
            const jitter = Math.floor(Math.random() * Math.min(200, delay / 2));
            const sleepMs = delay + jitter;
            console.error(`LLM request retry ${attempt}/${totalAttempts - 1} due to ${detail}; waiting ${sleepMs}ms...`);
