...
function getGlobalConfigDir() {
  ...
  const home = dntShim.Deno.env.get("HOME") ||
    dntShim.Deno.env.get("USERPROFILE");
  if (!home) {
...
export async function loadConfig({ cwd, cli, configPath }) {
  ...
  if (configPath) {
    try {
      ...
      const err = error;
      allErrors.push(
        `Failed to load config file ${configPath}: ${err.message}`,
      );
    }
  }
  ...
  if (cli.retryBackoffMs !== undefined) {
    cliConfig.retryBackoffMs = cli.retryBackoffMs;
  }
  if (cli.perFileTimeoutMs !== undefined) {
    cliConfig.perFileTimeoutMs = cli.perFileTimeoutMs;
  }
