 ...
 */
export async function processFile({ absPath, relPath, cfg, apiKey, dryRun, }) {
    const content = await dntShim.Deno.readTextFile(absPath);
    const { todos } = detectTodos(content);
    if (!todos.length)
        return { changed: 0, todosFound: 0 };
    ...
    if (updated === null)
        return { changed: 0, todosFound: todos.length };
    if (!dryRun) {
 ...
 */
async function rewriteTodos({ content, todos, relPath, cfg, apiKey, _dryRun, cache, fileStart, }) {
    let text = content;
    ...
    for (const todo of sorted) {
        ...
        if (Date.now() - fileStart > (cfg.perFileTimeoutMs ?? 120000)) {
            console.error(gray(`[timeout] file exceeded ${cfg.perFileTimeoutMs}ms, skipping remaining TODOs`));
            break;
        }
        ...
        if (cached) {
            if (cfg.cache && !cache[fileKey])
                cache[fileKey] = cached;
 ...
 */
function extractContext(content, todo, lines) {
