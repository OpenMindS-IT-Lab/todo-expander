 ...
 */
export async function discoverTargets({ cwd, mode, paths, include, exclude, maxFileKB, }) {
    if (mode === 'staged') {
        ...
        for (const file of abs) {
            try {
                ...
                if (!include.includes(ext))
                    continue;
                if (exclude.some((seg) => file.includes(`/${seg}/`)))
                    continue;
                if (info.size > maxFileKB * 1024)
                    continue;
                filtered.push(file);
            }
            catch (_) {
    ...
    for (const p of paths) {
        ...
        try {
            ...
            if (st.isFile) {
                ...
                if (!include.includes(ext))
                    continue;
                if (exclude.some((seg) => abs.includes(`/${seg}/`)))
                    continue;
                if (st.size > maxFileKB * 1024)
                    continue;
                results.push(abs);
            }
