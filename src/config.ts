...
export async function loadConfig({
  ...
  cwd: string
  cli: Partial<Cfg & { sections: string | string[]; include: string; exclude: string }>
}): Promise<Cfg> {
  const defaults: Cfg = {
  ...
  const resolved: Cfg = {
    ...defaults,
    include: cli.include ? cli.include.split(",").map((s) => s.trim()) : defaults.include,
    exclude: cli.exclude ? cli.exclude.split(",").map((s) => s.trim()) : defaults.exclude,
    style:
      (cli.style as Cfg["style"]) ||
