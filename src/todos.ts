...
/** Matches the start of a block TODO comment like `/* TODO: ... */`. */
...
export type TodoMatch = {
  ...
  raw: string
  /** Comment style: `line` for `//`/`#`, or `block` for `/* */`. */
  style: "line" | "block"
...
export function detectTodos(content: string) {
  ...
  const isStructured = (text: string) =>
    /AI TASK:|\bContext\b|\bGoal\b|\bSteps\b|\bConstraints\b|\bAcceptance\b/.test(text)
